...
class Client(object):
    ...
    def test_metrics(self):
        ...
        with torch.no_grad():
            if type(eval_data_x) == type([]):
                eval_data_x[0] = eval_data_x[0].to(self.device)
            else:
                eval_data_x = eval_data_x.to(self.device)
            eval_data_y = eval_data_y.to(self.device)
            output = self.model(eval_data_x)
            test_acc = (torch.sum(torch.argmax(output, dim=1) == eval_data_y)).item()
            test_num = eval_data_y.shape[0]
        with torch.no_grad():
