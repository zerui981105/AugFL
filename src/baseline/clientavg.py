...
from clientbase import get_input
...
class clientAVG(Client):
    ...
    def train(self):
        ...
        for epoch in range(max_local_epochs):
            ...
            #     loss.backward()
            #     self.optimizer.step()
            if type(train_data_x) == type([]):
                train_data_x[0] = train_data_x[0].to(self.device)
            else:
                train_data_x = train_data_x.to(self.device)
            train_data_y = train_data_y.to(self.device)
