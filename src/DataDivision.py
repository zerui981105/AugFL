...
import struct
import argparse
from STL10 import download_and_extract, read_all_images, read_labels
...
def get_data():
    # 文件获取
    train_image = "/root/autodl-tmp/datasets/emnist/emnist-letters-train-images-idx3-ubyte"
    test_image = "/root/autodl-tmp/datasets/emnist/emnist-letters-test-images-idx3-ubyte"
    train_label = "/root/autodl-tmp/datasets/emnist/emnist-letters-train-labels-idx1-ubyte"
    test_label = "/root/autodl-tmp/datasets/emnist/emnist-letters-test-labels-idx1-ubyte"
    # 读取数据
    train_x = read_Fmnist_image(train_image)
    test_x = read_Fmnist_image(test_image)
    train_y = read_Fmnist_label(train_label)
...
def prepare_cifar10():
    filenames = ['batches.meta', 'data_batch_1', 'data_batch_2', 'data_batch_3', 'data_batch_4', 'data_batch_5',
                 'test_batch']
    data_folder = '/root/autodl-tmp/datasets/cifar10/'
...
def prepare_cifar100():
    ...
    data_folder = '/root/autodl-tmp/datasets/cifar100/'
    data_paths = [os.path.join(data_folder, f) for f in filenames]
...
def genrate_cifar10(pretrain=False,user_num=50,a=10):
    ...
    generator=DataDivider(data_list,label_list,num_users=user_num,a=a,division_ratio=[0, 0, 0, 0, 1,0],train_test_ratio=0.5,savepath='/root/autodl-tmp/fmaml/fmaml_mac/data/cifar10_20',num_class=10)
...
def genrate_cifar100(user_num):
    ...
    generator=DataDivider(data_list,label_list,num_users=user_num,a=10,division_ratio=[0, 0, 0, 0,0, 1],train_test_ratio=0.5,savepath='/root/autodl-tmp/fmaml/fmaml_mac/data/cifar100',num_class=100)
    generator.save_data()

def generate_Emnist(user_num):
    data_list, label_list=prepare_Emnist_data()
    generator = DataDivider(data_list, label_list, num_users=user_num, a=10, division_ratio=[0, 0, 0, 0, 1, 0],
                            train_test_ratio=0.2, savepath='/root/autodl-tmp/fmaml/fmaml_mac/data/emnist', num_class=26)
    generator.save_data()
...
if __name__=='__main__':
    ...
    if parsed['dataset']=='cifar10':
        ...
        genrate_cifar10(pretrain=False, user_num=50, a=20)
    elif parsed['dataset']=='cifar100':
        print('Preparing cifar100')
        genrate_cifar100(100)
    elif parsed['dataset']=='Emnist':
        print('Preparing Emnist')
        generate_Emnist(100)
    else:
